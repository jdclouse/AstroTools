...
for p_idx = 1:length(planets)
    ...
    fig_handles{p_idx,4} = figure('Position', [0, 0, figWidth, figHeight]); % Coning angle plot
    fig_handles{p_idx,5} = figure('Position', [0, 0, figWidth, figHeight]); % SMA and ecc plot
    ...
    for b_idx = 1:length(beta_array)
        ...
        % Set options on functions
        pureTwoBodyPolar = @(t,X) polarProp(t,X,'Optimal',beta);
        sim_event = @(t,X) detectSemiMajorAxis(t,X,planet.a*.95);
        ...
        %
        % WARNING!! Ugly things can happen when this gets to close to
        % circular. If that happens, you'll have to raise the sma more.
        % 
        sim_event = @(t,X) detectSemiMajorAxis(t,X,planet.a);
        ...
        alpha_store2 = zeros(1,length(X_array2));
        for ii = 1:length(X_array2)
            [Xd, alpha] = polarProp(0,X_array2(ii,:)','RaisePeri',beta);
            alpha_store2(ii) = alpha;
        end
        alpha_store = [alpha_store alpha_store2];
        ...
        for ii = 1:length(X_array3)
            [Xd, alpha] = polarProp(0,X_array3(ii,:)','Perpendicular',beta);
        ...
        e_store = [];
        for ii = 1:length(X_array)
            r = Euler2DCM('3',X_array(ii,2))*[X_array(ii,1);0;0];
            v = Euler2DCM('3',X_array(ii,2))*[X_array(ii,3); ...
                                    (X_array(ii,1)*X_array(ii,4));...
                                     0];
            [a_store(ii),e_store(ii),~,~,~,~] = cart2OE(r,v,Sun.mu);
        end
        ...
        % SMA
        figure(fig_handles{p_idx,5})
        subplot(1,1)
        plot(X_array(:,2)/2/pi,a_store/au2km, line_styles{b_idx},...
            'LineWidth',2); hold on;
        
        % ecc
        figure(fig_handles{p_idx,5})
        subplot(1,2)
        plot(X_array(:,2)/2/pi,e_store, line_styles{b_idx},...
            'LineWidth',2); hold on;
...
%     title(['Sun Coning Angle for ' planet.name ' Transfer'])
    ...
    legend(sprintf('\\beta=%.2f',beta_array(1)), ...
        sprintf('\\beta=%.2f',beta_array(2)), ...
        sprintf('\\beta=%.2f',beta_array(3)))
    print(['Webpage/images/a_e_' planet.name 'ConeAngles'],'-dpng')
end