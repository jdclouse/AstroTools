...
for nn = prnlist  % Loop through satellite list ---------------------------
    ...
    for tt = 1:sz % loop through all times ------------------------
        ...
        if (health(tt) ~= 0)   % satellite is unhealthy
            ...
            if GPSdata.topo_el(tt,nn) < 0.0
                GPSdata.topo_el(tt,nn) = NaN;
                GPSdata.topo_az(tt,nn) = NaN;
            end
...
for tt = 1:sz
    ...
    %Loop through all the PRNs and ignore elevation below defined mask
    %angle
    
    for nn = 1:length(prnlist)
        if GPSdata.topo_el(tt,nn) >= topomaskmin
            %Valid visibility, increment satellites visible
            nSatVisCount = nSatVisCount + 1;
        end
    end
    GPSdata.ant_numsats(tt) = nSatVisCount;
